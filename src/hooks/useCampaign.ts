...
export function useCampaign(campaignAddress: string) {
  ...
  const { data: imageHash } = useReadContract({ contract, method: "function imageHash() view returns (string)" });
  const { data: goal } = useReadContract({ contract, method: "function goal() view returns (uint256)" });
  const { data: deadline } = useReadContract({ contract, method: "function deadline() view returns (uint256)" });
  const { data: balance } = useReadContract({ contract, method: "function getContractBalance() view returns (uint256)" });
  const { data: state } = useReadContract({ contract, method: "function state() view returns (uint8)" });
  const { data: owner } = useReadContract({ contract, method: "function owner() view returns (address)" });
  const { data: paused } = useReadContract({ contract, method: "function paused() view returns (bool)" });
  const { data: tiers } = useReadContract({ contract, method: "function getTiers() view returns ((string,string,uint256,uint256,string)[])" });
  ...
  useEffect(() => {
    if (name && description && imageHash && goal !== undefined && 
        deadline !== undefined && balance !== undefined && 
        state !== undefined && owner && paused !== undefined && tiers) {
      setCampaignData({
        name,
        description,
        imageHash,
  ...
  }, [name, description, imageHash, goal, deadline, balance, state, owner, paused, tiers]);
