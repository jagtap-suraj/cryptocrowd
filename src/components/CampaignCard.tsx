...
export function CampaignCard({ campaignAddress, name, imageHash }: CampaignCardProps) {
  ...
  const imageUrl = useMemo(() => {
    ...
    try {
      ...
      if (processedImageHash.startsWith("http://") || processedImageHash.startsWith("https://")) {
        ...
        <div className="p-4">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5 mb-2">
            <div 
              ...
              style={{ 
                width: `${campaignDetails ? Math.min(
          ...
          <div className="flex justify-between text-sm">
            <div>
              <p className="font-medium">{campaignDetails ? 
                `${(Number(campaignDetails.balance) / 1e18).toFixed(4)} ETH` : '0 ETH'}</p>
              <p className="text-gray-500">raised of {campaignDetails ? 
                `${(Number(campaignDetails.goal) / 1e18).toFixed(2)} ETH` : '0 ETH'}</p>
            </div>
            <div>
              <p className="font-medium">{campaignDetails ? 
                new Date(Number(campaignDetails.deadline) * 1000).toLocaleDateString() : 'N/A'}</p>
          ...
          {campaignDetails && (
            <div className={`mt-3 inline-block px-2 py-1 rounded text-xs font-medium
              ${campaignDetails.state === 0 ? 'bg-green-100 text-green-800' : 
                campaignDetails.state === 1 ? 'bg-blue-100 text-blue-800' : 
                'bg-red-100 text-red-800'}`}>
              {campaignDetails.state === 0 ? 'Active' : 
               campaignDetails.state === 1 ? 'Successful' : 'Failed'}
            </div>
          )}
        </div>
      </div>
